...
import org.sakaiproject.site.api.SiteServiceSql;
...
public class SiteServiceSqlDefault implements SiteServiceSql {
    ...
    */
/* public String getSitesCountWhere12Sql() {
      return "SAKAI_SITE.SITE_ID not in (select SITE_ID from SAKAI_SITE_USER where USER_ID = ? and PERMISSION <= PUBLISHED) and ";
   }                    "SITE_ID not in (select SITE_ID from SAKAI_SITE_USER where USER_ID = ? and PERMISSION <= PUBLISHED) and "
*/
   /**
   ...
   public String getToolFields1Sql() {
      return "select REGISTRATION, SAKAI_SITE_TOOL.TITLE, LAYOUT_HINTS, SAKAI_SITE_TOOL.SITE_ID, PAGE_ID, SKIN, PUBLISHED, PAGE_ORDER " +
             "from   SAKAI_SITE_TOOL, SAKAI_SITE " +
   ...
   public String getPageFields1Sql() {
      return "select PAGE_ID, SAKAI_SITE_PAGE.TITLE, LAYOUT, SAKAI_SITE_PAGE.SITE_ID, SKIN, PUBLISHED, POPUP " +
              "from  SAKAI_SITE_PAGE, SAKAI_SITE " +
              "where SAKAI_SITE_PAGE.SITE_ID = SAKAI_SITE.SITE_ID " + "and PAGE_ID = ?";
   }
